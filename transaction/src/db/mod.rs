...
fn create_balance<'a>(conn: &PgConnection, iban: &'a str, token: &'a str, type_: &'a str) -> Balance {
...
fn get_balance_by_iban(conn: &PgConnection, ib: &str) -> Option<Balance> {
    balance::table.filter(balance::iban.eq(ib)).first::<Balance>(conn).optional().unwrap()
}

pub fn get_cac(conn: &PgConnection, id: String, tp: &str) -> Cac {
    match cacr::table.find(id.clone()).first::<Cac>(conn).optional() {
        Ok(Some(v)) => v,
        Ok(None) => create_cac(conn, id, tp),
        Err(e) => panic!("Error trying to get cac with uuid: {:?} and error: {}", id, e)
    }
}

pub fn get_cmt(conn: &PgConnection, id: String, values: &[(String, Value)]) -> (Cmt, Option<Balance>, Option<Balance>) {
    match cmtr::table.find(id.clone()).first::<Cmt>(conn).optional() {
...
fn create_cac(conn: &PgConnection, uuid: String, tp: &str) -> Cac {
    let iban = new_iban();
...
fn create_cmt(conn: &PgConnection, uuid: String, values: &[(String, Value)]) -> (Cmt, Option<Balance>, Option<Balance>) {
